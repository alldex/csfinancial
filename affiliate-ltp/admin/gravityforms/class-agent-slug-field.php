 ...
 * @author snielson
 */
class Agent_Slug_Field extends GF_Field {
    
    /**
     * Unique name of the type of field this is for gravity forms.
     * @var type 
     */
    public $type = 'agent-slug';
    
    /**
     * Holds the value of the agent slug when a valid registration occurs so it
     * can be used to connect the agent to other pieces of data (such as the
     * parent agent).
     * @var type 
     */
    private $slug_value = null;
    ...
    public function get_form_editor_field_title() {
        return esc_attr__('Agent Slug', 'affiliate-ltp');
    }
...
//            'phone_format_setting',
    ...
    public function get_value_save_entry($value, $form, $input_name, $lead_id, $lead) {
        ...
        if (!$this->failed_validation) {
            // setup the hook so we can return the correct agent_id here...
            // give it a lower priority so we can be last
            $this->slug_value = $value;
            add_filter('affwp_tracking_get_affiliate_id', array($this, 'return_agent_id_from_slug'), 20, 2);
        }
        return $value;
    }
